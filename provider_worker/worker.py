...
def run_once(base_url:str,secret:str,token:str|None,worker_id:str,cloud_post=post_json,duffel_transport=duffel.default_transport):
    ...
    for job in leased:
        try:
            ...
            normalized=[]
            for offer in sorted(found['offers'],key=lambda o:float(o.get('total_amount','inf')))[:10]:
                snap=duffel.normalize_offer(offer,query,qfp,job['job_id'],now); cloud_post(base_url,'/offers/ingest',snap,secret); normalized.append(snap['provider_offer_id'])
