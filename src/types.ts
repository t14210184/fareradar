...
export interface CostComponentInput { cost_id:string; type:string; amount:number; currency:string; twd_amount:number|null; inclusion_state:"INCLUDED_IN_OFFER"|"ADD_ON"|"UNKNOWN"; source_offer_id?:string|null; dedupe_key:string; certainty:string; effective_event_at?:string|null; paid_state:string; refundable:boolean; fx_snapshot_id?:string|null; observed_at:string; }
...
export interface CandidatePlanInput { intake_id:string; itinerary:ItineraryCandidateInput; tickets:TicketComponentInput[]; transfers:TransferBoundaryInput[]; costs:CostComponentInput[]; readiness:ReadinessFacetInput[]; documents:DocumentRequirementInput[]; four_leg_liabilities:FourLegLiabilityInput[]; }
