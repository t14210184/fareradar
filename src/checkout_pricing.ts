...
export async function applyPricingQuoteEvidence(db:D1Database,quoteId:string,nowIso:string){
  ...
  for(const r of rows){const twd=q.currency==="TWD"?Number(q.grand_total):null;await db.batch([
      db.prepare("UPDATE cost_components SET type='CHECKOUT_TOTAL',amount=?,currency=?,twd_amount=?,inclusion_state='INCLUDED_IN_OFFER',certainty='CHECKOUT_REPRICE',pricing_quote_id=?,observed_at=?,evidence_expires_at=? WHERE itinerary_id=? AND source_offer_id=? AND dedupe_key='offer-total'").bind(q.grand_total,q.currency,twd,q.quote_id,q.priced_at,q.expires_at??null,r.itinerary_id,q.provider_offer_id),
