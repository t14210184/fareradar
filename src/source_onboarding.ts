...
export async function applySourceOnboardingReview(db:D1Database,r:SourceOnboardingReview,nowIso:string){
  ...
  const ev=await db.prepare("SELECT gate_id,spec_version,unresolved_items FROM audit_evidence WHERE evidence_id=?").bind(r.evidence_id).first<any>();
  if(!ev||ev.spec_version!=="1.3")throw new Error("SOURCE_REVIEW_EVIDENCE_INVALID");
  if((r.target_state==="SHADOW"&&ev.gate_id!=="PG25")||(r.target_state==="ENABLED"&&ev.gate_id!=="PG31"))throw new Error("SOURCE_REVIEW_GATE_MISMATCH");
  ...
  const canonical=JSON.stringify({review_id:r.review_id,source_id:r.source_id,target_state:r.target_state,terms_snapshot_at:r.terms_snapshot_at,evidence_id:r.evidence_id,checks});
  const payloadHash=await sha256Hex(canonical);
  const prior=await db.prepare("SELECT payload_sha256 FROM source_onboarding_reviews WHERE review_id=?").bind(r.review_id).first<{payload_sha256:string}>();
  ...
  await db.batch([
    db.prepare("INSERT INTO source_onboarding_reviews(review_id,source_id,target_state,terms_snapshot_at,evidence_id,checks_json,payload_sha256,created_at) VALUES(?,?,?,?,?,?,?,?)").bind(r.review_id,r.source_id,r.target_state,r.terms_snapshot_at,r.evidence_id,JSON.stringify(checks),payloadHash,nowIso),
