...
export async function leaseProviderJobs(db:D1Database,input:{provider_id:string;worker_id:string;limit?:number},nowIso:string){
  const ready=await providerReady(db,{provider_id:input.provider_id,worker_id:input.worker_id,verification_type:"LIVE_REPRICE"},nowIso);if(!ready.ready)throw new Error(ready.reason);
  ...
  const until=new Date(Date.parse(nowIso)+90_000).toISOString();
  return (await db.prepare(`UPDATE verification_jobs SET state='LEASED',claimed_by=?,lease_until=?,attempts=attempts+1
    WHERE job_id IN (SELECT job_id FROM verification_jobs WHERE target_class='PROVIDER_API' AND provider_id=? AND state='PENDING' AND available_at<=? AND (provider_mode='USER_REQUEST' OR ?=1) ORDER BY created_at LIMIT ?)
