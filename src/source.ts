...
export interface PromotionFingerprintInput {market:string;airline?:string;routes:string[];sale_start?:string;sale_end?:string;travel_start?:string;travel_end?:string;promo_code?:string;member_requirement?:string|null;channel_requirement?:string|null;sale_direction?:string|null;required_roundtrip?:boolean;eligible_flight_numbers?:string[];eligible_weekdays?:number[];blackout_dates?:string[];origin_market?:string|null;member_only?:boolean;subscription_only?:boolean;coupon_required?:boolean;minimum_stay?:number|null;maximum_stay?:number|null;}
export function promotionFingerprint(p:PromotionFingerprintInput){const rules={sale_end:p.sale_end??"",travel_end:p.travel_end??"",sale_direction:p.sale_direction??"",required_roundtrip:!!p.required_roundtrip,eligible_flight_numbers:[...(p.eligible_flight_numbers??[])].sort(),eligible_weekdays:[...(p.eligible_weekdays??[])].sort((a,b)=>a-b),blackout_dates:[...(p.blackout_dates??[])].sort(),origin_market:p.origin_market??"",member_requirement:p.member_requirement??"",channel_requirement:p.channel_requirement??"",member_only:!!p.member_only,subscription_only:!!p.subscription_only,coupon_required:!!p.coupon_required,minimum_stay:p.minimum_stay??null,maximum_stay:p.maximum_stay??null};return [p.market,p.airline??"",[...p.routes].sort().join(","),p.sale_start??"",p.travel_start??"",p.promo_code??"",JSON.stringify(rules)].join("|").toLowerCase();}
...
export interface PromotionSignal { market:string; routes:string[]; prices:{currency:string;amount:number}[]; promo_code:string|null; keywords:string[]; member_requirement:string|null; channel_requirement:string|null; }
export function extractPromotionText(text:string,market="TW"):PromotionSignal|null {
  ...
  return {market,routes:[...new Set(routes)].sort(),prices,promo_code:promo?.[1]?.toUpperCase()??null,keywords,member_requirement,channel_requirement};
}
