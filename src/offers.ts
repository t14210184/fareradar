...
export interface OfferSnapshotInput { provider_offer_id:string; itinerary_id?:string|null; query_fingerprint:string; provider:string; market?:string|null; locale?:string|null; currency:string; passenger_mix?:string|null; baggage_query?:string|null; observed_at:string; expires_at?:string|null; raw_sha256:string; source_snapshot_id?:string|null; offer_total:number; fare_freshness:string; cached_or_live:"CACHED"|"LIVE"; contains_lcc?:boolean; }
export async function ingestOfferSnapshot(db:D1Database,o:OfferSnapshotInput){
  ...
  await db.prepare("INSERT INTO offer_snapshots(provider_offer_id,itinerary_id,query_fingerprint,provider,market,locale,currency,passenger_mix,baggage_query,observed_at,expires_at,raw_sha256,source_snapshot_id,offer_total,fare_freshness,cached_or_live) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
    .bind(o.provider_offer_id,o.itinerary_id??null,o.query_fingerprint,o.provider,o.market??null,o.locale??null,o.currency,o.passenger_mix??null,o.baggage_query??null,o.observed_at,o.expires_at??null,o.raw_sha256,o.source_snapshot_id??null,o.offer_total,o.fare_freshness,o.cached_or_live).run();
