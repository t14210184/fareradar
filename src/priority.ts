...
export async function ackCandidateSignal(db:D1Database,input:{queue_id:string;ok:boolean;retryable?:boolean;error?:string|null},nowIso:string){
  const row=await db.prepare("SELECT attempts,signal_id FROM candidate_priority_queue WHERE queue_id=?").bind(input.queue_id).first<{attempts:number;signal_id:string}>();
  if(!row)throw new Error("CANDIDATE_SIGNAL_NOT_FOUND");
  if(input.ok){await db.prepare("UPDATE candidate_priority_queue SET state='DONE',claimed_by=NULL,lease_until=NULL,last_error=NULL,updated_at=? WHERE queue_id=?").bind(nowIso,input.queue_id).run();return "DONE";}
  ...
  await db.prepare("UPDATE candidate_priority_queue SET state='DEAD',claimed_by=NULL,lease_until=NULL,last_error=?,updated_at=? WHERE queue_id=?").bind(input.error??"TERMINAL",nowIso,input.queue_id).run();
