...
export async function expireAgencyOffers(db:D1Database,nowIso:string,limit=1){
  ...
  for(const row of rows){
    ...
    await db.batch([
      ...
      db.prepare("UPDATE notification_outbox SET state='CANCELLED',lease_until=NULL,last_error=? WHERE notification_id IN (SELECT intent_id FROM candidate_alert_intents WHERE itinerary_id=?) AND state IN ('PENDING','SHADOW_HELD')").bind(x.reason,`agency:${row.agency_offer_id}`)
    ]);
