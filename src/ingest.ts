...
async function projectPromotionSignal(db:D1Database,obs:{observation_id:string;observed_at:string},p:any,nowIso:string){
  const fp=promotionFingerprint({market:p.market??"UNKNOWN",airline:p.airline,routes:p.routes??[],sale_start:p.sale_start,travel_start:p.travel_start,promo_code:p.promo_code,member_requirement:p.member_requirement??null,channel_requirement:p.channel_requirement??null}); const id=eventIdFromFingerprint(fp);
  await db.batch([
