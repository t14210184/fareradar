...
import { projectFareBaselineFromOffer } from "./baseline.js";
...
async function projectPromotionSignal(db:D1Database,obs:{observation_id:string;observed_at:string},p:any,nowIso:string){
  const constraint={sale_direction:p.sale_direction??p.direction??null,required_roundtrip:!!p.required_roundtrip,eligible_flight_numbers:[...(p.eligible_flight_numbers??p.flight_numbers??[])].sort(),eligible_weekdays:[...(p.eligible_weekdays??p.weekdays??[])].sort((a:number,b:number)=>a-b),blackout_dates:[...(p.blackout_dates??[])].sort(),origin_market:p.origin_market??p.market??null,sales_currency:p.sales_currency??(p.prices??[])[0]?.currency??null,member_only:!!p.member_only,subscription_only:!!p.subscription_only,coupon_required:!!p.coupon_required,minimum_stay:p.minimum_stay??null,maximum_stay:p.maximum_stay??null,stopover_allowed:p.stopover_allowed??null,open_jaw_allowed:p.open_jaw_allowed??null,change_rule:p.change_rule??null,refund_rule:p.refund_rule??null,fare_brand:p.fare_brand??null,baggage_bundle:p.baggage_bundle??null};
  const fp=promotionFingerprint({market:p.market??"UNKNOWN",airline:p.airline,routes:p.routes??[],sale_start:p.sale_start,sale_end:p.sale_end,travel_start:p.travel_start,travel_end:p.travel_end,promo_code:p.promo_code,member_requirement:p.member_requirement??null,channel_requirement:p.channel_requirement??null,...constraint}); const id=eventIdFromFingerprint(fp);
  await db.batch([
    db.prepare("INSERT INTO promotion_events(event_id,fingerprint,state,market,airline,routes_json,prices_json,promo_code,observed_at,updated_at,promotion_type,carrier_or_seller,route_scope,sale_window,travel_window,price_claim,currency,member_requirement,channel_requirement,first_observed_at,last_observed_at,cluster_fingerprint,primary_evidence_id,constraint_json) VALUES(?,?,'DISCOVERED',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(fingerprint) DO UPDATE SET updated_at=excluded.updated_at,last_observed_at=excluded.last_observed_at").bind(id,fp,p.market??null,p.airline??null,JSON.stringify(p.routes??[]),JSON.stringify(p.prices??[]),p.promo_code??null,obs.observed_at,nowIso,p.promotion_type??'FARE_PROMOTION',p.airline??p.seller??null,JSON.stringify(p.routes??[]),JSON.stringify({start:p.sale_start??null,end:p.sale_end??null}),JSON.stringify({start:p.travel_start??null,end:p.travel_end??null}),JSON.stringify(p.prices??[]),(p.prices??[])[0]?.currency??null,p.member_requirement??null,p.channel_requirement??null,obs.observed_at,obs.observed_at,fp,obs.observation_id,JSON.stringify(constraint)),
...
export async function projectDomainEvents(db:D1Database,nowIso:string,workerId="cron",limit=2){
  ...
  for(const e of events as any[]){
    try{
      ...
      } else if(e.event_type==="OFFER_SNAPSHOT"){
        await projectFareBaselineFromOffer(db,e.entity_id,nowIso);
      }
