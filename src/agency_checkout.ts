...
export async function leaseAgencyCheckouts(db:D1Database,input:{agency_id:string;worker_id:string;limit?:number},nowIso:string){
  ...
  return (await db.prepare(`UPDATE agency_checkout_jobs SET state='LEASED',claimed_by=?,lease_until=?,attempts=attempts+1,updated_at=?
    WHERE job_id IN (SELECT job_id FROM agency_checkout_jobs j2 JOIN agency_inventory_offers a2 ON a2.agency_offer_id=j2.agency_offer_id WHERE j2.agency_id=? AND j2.state='PENDING' AND j2.available_at<=? AND a2.state NOT IN ('EXPIRED','SOLD_OUT') AND (a2.seats_available IS NULL OR a2.seats_available>0) AND (a2.booking_deadline IS NULL OR datetime(a2.booking_deadline)>datetime(?)) AND (a2.payment_deadline IS NULL OR datetime(a2.payment_deadline)>datetime(?)) AND (a2.ticketing_deadline IS NULL OR datetime(a2.ticketing_deadline)>datetime(?)) ORDER BY created_at ASC LIMIT ?)
    AND state='PENDING' RETURNING job_id,agency_offer_id,agency_id,attempts,lease_until`)
    .bind(input.worker_id,until,nowIso,input.agency_id,nowIso,nowIso,nowIso,nowIso,limit).all<any>()).results;
}
...
export interface AgencyCheckoutInput {
  ...
  seats_available?:number|null;booking_deadline?:string|null;payment_deadline?:string|null;total_includes_taxes:boolean;total_includes_mandatory_fees:boolean;
...
export async function completeAgencyCheckout(db:D1Database,input:AgencyCheckoutInput,nowIso:string){
  ...
  const row=await db.prepare(`SELECT j.state,j.claimed_by,j.lease_until,j.attempts,j.agency_offer_id,j.agency_id,a.currency,a.product_id,a.tax_inclusion,a.baggage,a.booking_or_contact_channel,a.booking_deadline existing_booking_deadline,a.payment_deadline existing_payment_deadline,a.ticketing_deadline existing_ticketing_deadline,a.state offer_state
  ...
  const deadlines=[input.booking_deadline??row.existing_booking_deadline,input.payment_deadline??row.existing_payment_deadline,row.existing_ticketing_deadline].filter((x:any)=>x&&Number.isFinite(Date.parse(x)));
  if(deadlines.some((x:any)=>Date.parse(x)<=Date.parse(nowIso))){result="RECHECK_REQUIRED";error="AGENCY_CHECKOUT_DEADLINE_PASSED";}
  ...
  await db.prepare(`INSERT INTO agency_checkout_evidence(checkout_id,job_id,agency_offer_id,agency_id,worker_id,readback_basis,checkout_url,content_sha256,observed_at,final_price,currency,seats_available,booking_deadline,payment_deadline,total_includes_taxes,total_includes_mandatory_fees,payment_dispatched,result_state,created_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(input.checkout_id,input.job_id,input.agency_offer_id,input.agency_id,input.worker_id,input.readback_basis,input.checkout_url,input.content_sha256,input.observed_at,input.final_price,input.currency,input.seats_available??null,input.booking_deadline??null,input.payment_deadline??null,input.total_includes_taxes?1:0,input.total_includes_mandatory_fees?1:0,0,result,nowIso).run();
  ...
  await db.prepare("UPDATE agency_inventory_offers SET seller_verification_state='CHECKOUT_REPRODUCED',price=?,seats_available=COALESCE(?,seats_available),booking_deadline=COALESCE(?,booking_deadline),payment_deadline=COALESCE(?,payment_deadline),observed_at=?,state='CHECKOUT_REPRODUCED' WHERE agency_offer_id=?")
    .bind(input.final_price,input.seats_available??null,input.booking_deadline??null,input.payment_deadline??null,input.observed_at,input.agency_offer_id).run();
