...
def test_run_once_fetch_ingest_complete(monkeypatch):
    ...
    def post(base,path,payload,secret):
        ...
        return {'ok':True}
    monkeypatch.setattr(ew,'post_json',post); monkeypatch.setattr(ew,'fetch',lambda url:{'status':200,'url':url,'body':'TPE-KIX 限時特價 NT$3,999','content_sha256':'a'*64,'etag':'"e"','last_modified':None,'content_type':'text/html'})
...
def test_run_once_failure_reports_completion(monkeypatch):
    ...
    monkeypatch.setattr(ew,'post_json',post); monkeypatch.setattr(ew,'fetch',lambda url:(_ for _ in ()).throw(ValueError('MIME_BLOCKED')))
